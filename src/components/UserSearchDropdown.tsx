...
export function UserSearchDropdown({ onSelect, placeholder = "Search by name or email…", autoFocus, disabled }: UserSearchDropdownProps) {
  ...
  const showPanel = open && (loading || results.length > 0 || query.trim().length > 0);
    ...
    <div className="user-search" ref={rootRef}>
      ...
      {showPanel && (
        <ul className="user-search-results" id={listboxId} role="listbox">
          {loading && results.length === 0 && <li className="user-search-empty">Searching…</li>}
          {!loading && results.length === 0 && query.trim().length > 0 && (
            <li className="user-search-empty">No one found for &ldquo;{query.trim()}&rdquo;</li>
          )}
          {results.map((user, i) => (
            <li
              key={user.id}
            ...
            >
              <Avatar initials={initialsOf(user.displayName)} color={colorFor(user.id)} size="sm" />
              <div>
